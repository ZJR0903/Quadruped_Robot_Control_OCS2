...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh)
{
  ...
  /// when verbose is true, terminal will print a lot of debug message
  bool verbose;
  ...
  mpc_ = std::make_shared<MultipleShootingMpc>(legged_interface_->mpcSettings(), legged_interface_->sqpSettings()/*set MPC\SQP file*/,
                                               legged_interface_->getOptimalControlProblem(),
                                               legged_interface_->getInitializer());
  /// upload centroidal model 
  /// nonlinear algorithm
  ...
  /// manages both ModeSchedule and TargetTrajectories
  mpc_->getSolverPtr()->setReferenceManager(ros_reference_manager_ptr);
  /// publish mpc observise
